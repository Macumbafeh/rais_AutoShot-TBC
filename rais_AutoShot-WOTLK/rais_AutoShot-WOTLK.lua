...
local AddOn = "rais_AutoShot-WOTLK"
local _G = getfenv(0)
...
function r_Latency(arg)
    if not r then
        print(AddOn .. ": Settings not loaded yet!")
        return
    end
...
Frame:SetScript("OnEvent",function()
	...
	if ( event == "PLAYER_LOGIN" ) then
	 if type(raisAutoShot) ~= "table" then
        raisAutoShot = {} -- Initialize if undefined
    end
    r = raisAutoShot
    r.w = r.w or (Table["Width"] * GetScreenWidth() / 1000)
    r.h = r.h or (Table["Height"] * GetScreenHeight() / 1000)
...
Frame:SetScript("OnEvent", function(self, event, addon)
    if event == "ADDON_LOADED" and addon == "rais_AutoShot-WOTLK" then
        if type(raisAutoShot) ~= "table" then
            raisAutoShot = {} -- Initialize saved variables if nil
        end
        r = raisAutoShot
        r.w = r.w or (Table["Width"] * GetScreenWidth() / 1000)
        r.h = r.h or (Table["Height"] * GetScreenHeight() / 1000)
...
Frame:SetScript("OnEvent", function(self, event)
    if event == "PLAYER_LOGOUT" then
        raisAutoShot = r -- Save the current settings
    end
end)
